...
def sequence_data(data_dir, n_nodes, n_timesteps_per_day, n_timesteps_in, n_timesteps_out, n_features_in):
    ...
    Returns:
        np.ndarray: input and labels for the model, with shape (n_sequences, n_timesteps_in + n_timesteps_out, n_nodes, n_features_in)
    """

    dataset = pd.read_csv("./data/nyc_pickups.csv")
    ...
    for i in range(n_days):
        ...
        for j in range(n_slot-1):
            ...
            tmp_data = np.concatenate((input_seq, output_seq), axis=0)  # Concatenate along the time axis
            dataset_seq[counter] = np.expand_dims(tmp_data, axis=-1)  # Expand dims to match (n_timesteps_seq, n_nodes, n_features_in)
            counter += 1
...
def process_adjacency(data_dir, dataset, ks, n_nodes, approx, device):
    ...
    """
    A = np.array(pd.read_csv("./data/nyc_adj.csv"))
...
def scaled_laplacian(W):
    W = W.astype(np.float64)  # Ensure W is of type float64
    print("DIMENSIONS OF ADJ MAT = ", W.shape)
...
def cheb_poly_approx(L, Ks, n):
    ...
    L1 = np.mat(np.copy(L))
    print(f"Initial L0 shape: {L0.shape}, L1 shape: {L1.shape}, L shape: {L.shape}")
    ...
    if Ks > 1:
        ...
        for i in range(Ks - 2):
            ...
            print(f"Iteration {i}: Ln shape: {Ln.shape}")
            L0, L1 = np.copy(L1), np.copy(Ln)
        # L_list [Ks, n*n], Lk [n, Ks*n]
