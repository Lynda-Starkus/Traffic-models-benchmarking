...
class TemporalConvLayer(nn.Module):
    ...
    def forward(self, X):
        ...
        P = x_conv[:, 0:self.c_out, :x_residual.shape[2], :]
        Q = x_conv[:, -self.c_out:, :x_residual.shape[2], :]

        print("Shape of P : ", x_residual.shape[2])
...
class OutputLayer(nn.Module):
    ...
    def forward(self, X):
        ...
        X = X[:, :self.n_in, :, :]
        print("shape of X in output layer", X.shape)
